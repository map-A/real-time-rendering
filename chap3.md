...
Geometry shader可以将一个基元转换成另一个基元，几何着色器的输入是一个单独的对象及其关联的顶点。

前三个Geometry shader的输入分别是点，线，三角形。另外，后面两种一条线附加两个点，一个三角形附加外面3个点也是可以作为输入的。

![image-20230811155803501](chap3.assets/image-20230811155803501.png)

Geometry shader处理完基元后可能输出基元也可能不输出，可以通过编辑顶点、添加新的基元和删除其他基元来选择性地修改网格。几何着色器旨在修改输入数据或创建有限数量的副本。下图是GS的一些用途。还有很多用途。

![image-20230811161029683](chap3.assets/image-20230811161029683.png)

DirectX11 增加了GS使用示例的能力，GS可以在任何给定的基元上运行一定的次数，GS还可以一次输出多达4个stream，一个stream可以送到渲染管线进一步处理，所有的这些流可以送到输出渲染目标。

在发出`draw`调用之后，管线中只有三个位置可以做创建类的工作：光栅化、曲面细分和GS。GS是可选完全可编程的，它的未来使用需求（内存，资源）最不可预测，很少使用

### 3.7.1 Stream Output

一般GPU的渲染管线的流程是：顶点着色器处理顶点数据，然后生成的三角形光栅化，最后在像素着色器中填充颜色，没有外部行为进行干涉这个流程。

在Shader Model 4.0中引入了流输出的概念。在顶点着色器（和可选的细分和几何着色器）处理顶点后（光栅化前），这些顶点可以按顺序输出到一个流中，然后送给IA、VS、HS、TS、DS、GS，可以依次迭代复用处理。

## 3.8 The Pixel Shader

在GS之后，就是光栅化。管线的这一部分在其处理步骤上相对固定，即不可编程但在一定程度上可配置。光栅化遍历三角形确定了覆盖的像素。三角形顶点的值（包括 z 缓冲区中使用的 z 值）针对每个像素在三角形表面上进行插值。这些值被传递到像素着色器，然后像素着色器处理fragment。

像素着色器的输入来源于顶点着色器的输出三角形，也可以是其他的输入。三角形的可见面是一个输入标志。对于在单次渲染中在每个三角形的前面和后面渲染不同材质。

像素着色器对每个输入进行计算然后输出（颜色+透明度值），z-深度也可能修改，这些都是后面合并需要用到的。光栅化阶段生成的深度值也可以被像素着色器修改。模板缓冲区的值通常是不可修改的，而是传递到合并阶段。

**像素着色器还具有丢弃传入片段的能力，即不生成输出。**通过用户定义的剪切平面对模型进行裁剪体积。

###### ![image-20230811173914616](chap3.assets/image-20230811173914616.png)
...
像素着色器的限制是它通常只能在传递给它的fragment位置写入渲染目标，无法从相邻的像素读取当前结果。这个限制也有例外。一是像素着色器可以在计算梯度或导数信息期间立即访问相邻片段的信息（尽管是间接的）。通过计算xy方向的梯度值，可以知道图像右格子有多少被覆盖了像素。所有现代 GPU 都通过处理 2 × 2 组（称为quard）的片段来实现此功能。当像素着色器请求梯度值时，返回相邻片段之间的差异。下图

![image-20230811181042628](chap3.assets/image-20230811181042628.png)
...
UAV需要机制避免数据竞争，怕两个着色器修改同一索引位置的值。那就使用原子访问写入避免这个问题，但原子操作会引起停顿，因为它们等待另一个着色器对正在进行读取/修改/写入的内存位置进行访问。

**虽然原子操作可以避免数据冲突，但许多算法需要特定的执行顺序。**光栅化器顺序视图（ROVs）在DirectX 11.3中引入以强制执行执行顺序。**它们类似于UAV（无序访问视图）；它们可以以相同的方式被着色器读取和写入。关键的区别在于ROVs保证数据按正确的顺序访问。这大大增加了这些着色器可访问缓冲区的用途[327, 328]。例如，ROVs使得像素着色器能够编写自己的混合方法，因为它可以直接访问和写入ROV中的任何位置，因此不需要合并阶段[176]。代价是，如果检测到无序访问，像素 着色器调用可能会停顿，直到先前绘制的三角形被处理。

## 3.9 The Merge stage

**合并阶段是将在像素着色器中生成的深度和颜色fragment与帧缓冲区（显示到屏幕的缓存）合并的地方**。DirectX将此阶段称为**output merger**；OpenGL将其称为**per-sample operations**。

OM阶段做的工作：对透明的显示，会做颜色混合合成操作，对不透明的显示，替换颜色。

OM阶段可配置，但不可编程。可以做颜色混合透明度测试等等操作。最常见的是涉及颜色和alpha值的乘法、加法和减法的组合，但也可以进行其他操作，如最小值和最大值，以及位逻辑操作。DirectX 10增加了从像素着色器中混合两个颜色与帧缓冲颜色的能力。这种能力称为双源颜色混合，不能与多个渲染目标一起使用。多渲染目标否则支持混合，而DirectX 10.1引入了在每个单独缓冲区上执行不同混合操作的能力。



光栅化器有序视图 (ROV)和合并阶段都保证了绘制顺序，也就是输出不变性。

DirectX 11.3通过ROVs提供了一种可编程的混合方式，尽管性能有所牺牲。无论像素着色器结果的生成顺序如何，API要求结果必须按照输入的顺序进行排序并发送到合并阶段，按照对象和三角形的顺序进行处理。



## 3.10 The Compute Stage

**GPU有许多非图形化的用途。以这种方式使用硬件被称为GPU计算。**CUDA和OpenCL等平台被用来控制GPU作为一个大规模并行处理器，不需要或无法访问图形特定功能。这些框架通常使用C或C++等语言，并配合为GPU设计的库。
...
![image-20230814100653365](chap3.assets/image-20230814100653365.png)
